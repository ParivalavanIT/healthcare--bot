...
def tree_diagnosis(clf, feature_names, user_symptoms, num_days, severity_dict, description_dict, precaution_dict):
    ...
    def recurse(node, depth, user_symptoms, num_days):
        ...
        if tree_.feature[node] != _tree.TREE_UNDEFINED:
            ...
            if val <= threshold:
                recurse(tree_.children_left[node], depth + 1, user_symptoms, num_days)
            else:
                symptoms_present.append(name)
                recurse(tree_.children_right[node], depth + 1, user_symptoms, num_days)
        else:
            present_disease = print_disease(tree_.value[node])
            symptoms_exp = []  # Use Streamlit input elements to get symptoms
            second_prediction = sec_predict(symptoms_exp)
            condition_result = calc_condition(symptoms_exp, num_days, severity_dict)
    ...
    if choice == "Home":
        ...
        if st.sidebar.checkbox("Login"):
            if username == "Priyadharshini" and password == "samplepass":
                ...
                user_input = st.text_input("Enter your symptoms separated by comma")  # Get user symptoms
                num_days = st.number_input("From how many days?", min_value=1, key='num_days')  # Get number of days

                if st.button("Diagnose"):  # Button to trigger diagnosis
                    user_symptoms = user_input.split(",")  # Split user input into list of symptoms

                    # Obtain feature names from the training dataset columns
                    feature_names = list(training.columns[:-1])  # Assuming the last column is the target
